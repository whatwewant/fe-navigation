...
* @Last modified time: 2017-06-14T23:22:33+08:00
* @License: MIT
  ...
  return (
    <Flex width="100%" height="100%" className={styles.normal}>
      <View width={280} style={{ boxShadow: '0 5px 15px rgba(0,0,0,.08)' }}>
        <Navigation onSelectNav={props.onSelectNav} />
      </View>
      <View flex={1}>
...
const mapState = ({ navigation }) => {
  const { currentNav, list } = navigation;
  return {
    name: currentNav.name,
    list: Object.values(list[currentNav.key] || {}),
  };
};
